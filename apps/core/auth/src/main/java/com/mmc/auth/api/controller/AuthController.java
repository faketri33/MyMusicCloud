...
public class AuthController {
    ...
    @PostMapping("/login")
    public ResponseEntity<JwtState> login(@RequestBody AuthRequestDto ard){
        return ResponseEntity.ok(authService.login(ard));
    }

    @PostMapping("/register")
    public ResponseEntity<JwtState> register(@RequestBody AuthRequestDto ard){
        return ResponseEntity.ok(authService.register(ard));
    }
}
